...
public class LocationController extends GenericController {
    ...
    public ResponseEntity<LocationModel> findById(@RequestParam Long id){
        ...
        var model = command.execute(repositoryFactory.getRepository(LOCATION), LocationModel.class, id);
    ...
    public ResponseEntity<List<LocationModel>> findAll(){
        ...
        logger.info("{}", REQUEST);
        var model = command.execute(repositoryFactory.getRepository(LOCATION), LocationModel.class);
        logger.info(MESSAGE_PATTERN, MODEL, model);
        logger.info(END);
        return ok(model);
    }
    ...
    public ResponseEntity<GenericResponse> deleteById(@RequestParam(name = "locationId") Long id){
        ...
        var result = command.execute(repositoryFactory.getRepository(LOCATION), LOCATION, id);
